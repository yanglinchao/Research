...
for(cirulation in 1:20){
  ...
  # 先建立故障定位模型
  RFsite <- randomForest(y1~., data = subset(data_train, select = -c(y2)), ntree=ntree, importance=TRUE)
  ...
  def_Tag <- function(data, tag){
    ...
    resultdata <- cbind(data_x, data[, -grep("X", names(data))])
    return(resultdata)
  }
  ...
  # 使用故障定位模型对测试集进行故障定位
  predsite <- predict(RFsite, subset(data_test, select = -c(y1, y2)))
  # 根据定位结果给测试集打标签
