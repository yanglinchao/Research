...
for(cirulation in 1:50){
  ...
  def_Tag <- function(data, tag){
    ...
    woe <- matrix(data = c(2.188778, 2.36999, -2.67523,
                           -0.3686, -1.32868, 2.193678,
                           -1.50493, 0.311963, 1.364186,
                           1.281376, 0.213785, -0.83979),
                  nrow = 4, ncol = 3, byrow = TRUE)
    tag_matrix <- matrix(NA, nrow = nrow(data), ncol = ncol(woe))
