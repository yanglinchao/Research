...
for(cirulation in 1:100){
  ...
  # 进行预测
  pred <- predict(random, subset(data_test, select = -c(y)))
...
outputdata <- data.frame(num = apply(index_result, MARGIN = 2, mean),
                         ...
                         ntree = rep(ntree, 4))
write.csv(outputdata, paste(c("result_test1_RF_", ntree, "_word2vec_", "_", word2vec_vector_size, "_", word2vec_window, ".csv"), collapse = ""), row.names = FALSE)
