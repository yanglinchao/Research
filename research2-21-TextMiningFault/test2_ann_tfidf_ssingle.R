...
for(cirulation in 1:50){
  ...
  # 建立故障定位模型
  ANNsystem <- nnet(y1~., data = subset(data_train, select = -c(y2)), size = size, maxit = maxit, MaxNWts = MaxNWts)
  ...
  # 对测试集的故障部位进行预测
  predsite <- predict(ANNsystem, subset(data_test, select = -c(y1, y2)), type = "class")
